            ...
            loop {
                ...
                if !parts.is_empty() {
                    ...
                    for (idx, voltage) in parts.iter().enumerate() {
                            data_file.write(voltage.as_bytes()).unwrap();
                            data_file.write(b"\t").unwrap();
                        if idx == 0 {
                            let now: chrono::NaiveDateTime = Local::now().naive_local();
                            data_file.write(now.and_local_timezone(Local).unwrap().to_rfc3339().as_bytes()
